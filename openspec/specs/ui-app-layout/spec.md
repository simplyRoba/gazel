...
The root layout SHALL render the protected application shell for application routes and a standalone public surface for `/login`. Protected application content SHALL include navigation, a main content region, and pull-to-refresh feedback; the login route SHALL render without protected shell chrome. Pull-to-refresh feedback SHALL appear at the top of the viewport during eligible gestures and otherwise remain out of view.

#### Scenario: Layout renders navigation and content
- **WHEN** an authenticated application route other than `/login` is loaded
- **THEN** the layout SHALL render a navigation region and a `<main>` content region
- **AND** the active route's page content SHALL appear inside the content region

#### Scenario: Pull feedback is inactive
- **WHEN** the protected application layout renders and no pull gesture is active
- **THEN** no pull-to-refresh feedback SHALL be visible
...
- **AND** SHALL NOT load or expose protected application data before authentication is established
